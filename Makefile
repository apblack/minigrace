...
$(LIBRARY_WO_OBJECTDRAW:%.grace=js/%.gct): js/%.gct: modules/%.grace minigrace
	GRACE_MODULE_PATH=modules ./minigrace $(VERBOSITY) --make --target js --dir js $<

$(LIBRARY_WO_OBJECTDRAW:%.grace=js/%.js): js/%.js: js/%.gct
...
modules/dialect.grace modules/minitest.grace modules/notest.grace modules/requireTypes.grace modules/staticTypes.grace: standardGraceClass.gcn
...
$(OBJECTDRAW:%.grace=js/%.js): js/%.js: modules/%.grace js/dom.gct minigrace js/timer.gct js/standardGraceClass.gct
	GRACE_MODULE_PATH=js:modules ./minigrace --target js --dir js --make $(VERBOSITY) $<

$(OBJECTDRAW:%.grace=js/%.gct): js/%.gct: modules/%.grace js/dom.gct minigrace js/timer.gct js/standardGraceClass.gct
	GRACE_MODULE_PATH=js:modules ./minigrace --target js --dir js --make $(VERBOSITY) $<
